...
def fitness_pass_2_inputs(out, expected, inputs):
    if len(out) == 0:
        return -1000000.0
    if len(out) == 2:
        if (out[0] == expected[0] and out[1] == expected[1]) or (
            out[0] == expected[1] and out[1] == expected[0]
        ):
            return 0.0
        elif (out[0] == expected[0] and out[1] != expected[1]) or (
            out[0] != expected[0] and out[1] == expected[1]
        ):
            return -10.0
...
def fitness_int_float_sum(out, expected, inputs):
    if len(out) == 0:
        ...
        len(out) == 2 and out[0] in expected and out[1] in expected and out[0] != out[1]
    ):
        return -5.0
    elif (
        len(out) == 2
        and out[0] != expected[0]
        and out[1] != expected[1]
        and any([o in inputs for o in out])
    ):
...
def fitness_pass_inputs(out, expected, inputs):
    if len(out) == 0:
        return -1000000.0
    return (
        sum([i not in out for i in inputs]) * -100
        + sum([o not in inputs for o in out]) * -1000
    )
...
def fitness_1_4_B_pretraining(out, expected, inputs):
    if len(out) == 0:
        return -1000000.0
    expected_out_len = len(expected)
    fit = abs(len(out) - expected_out_len) * -200.0
    for o_idx, o in enumerate(out):
        if o_idx >= expected_out_len:
            o_idx = o_idx % expected_out_len
        fit += min(abs(o - expected[o_idx]), 100) * -100.0
    return fit
...
fitness_functions = {
    ...
    "9_2": fitness_boolean,
    "9_3": fitness_boolean,
    "10_1": fitness_boolean,
    "10_2": fitness_boolean,
    "10_3": fitness_boolean,
}
